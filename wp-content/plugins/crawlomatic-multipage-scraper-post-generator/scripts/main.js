"use strict"; 
var initial = '';
function crawlomaticLoading(btn)
{
...
function crawlomaticRmLoading(btn)
{
    ...
    function mainChanged()
    {
        ...
        if(jQuery("#spin_text option:selected").val() === 'best' || jQuery("#spin_text option:selected").val() === 'wordai' || jQuery("#spin_text option:selected").val() === 'spinrewriter' || jQuery("#spin_text option:selected").val() === 'turkcespin' || jQuery("#spin_text option:selected").val() === 'aiseo' || jQuery("#spin_text option:selected").val() === 'spinnerchief') 
        {      
        ...
        if(jQuery("#spin_text option:selected").val() === 'tldr' || jQuery("#spin_text option:selected").val() === 'best' || jQuery("#spin_text option:selected").val() === 'wordai' || jQuery("#spin_text option:selected").val() === 'spinrewriter' || jQuery("#spin_text option:selected").val() === 'turkcespin' || jQuery("#spin_text option:selected").val() === 'aiseo' || jQuery("#spin_text option:selected").val() === 'spinnerchief') 
        {      
...
if (mycustommainsettings.best_user == '' || mycustommainsettings.best_password == '') 
{
        ...
        if(jQuery("#spin_text option:selected").val() === 'aiseo') 
        {      
            jQuery("#aiseo").show();
        }
        else
    ...
    jQuery(document).ready(function()
    {
        ...
        jQuery('#' + plugin_slug + '_register').on('click', function()
        {
            ...
            else
            {
                ...
                crawlomaticLoading(thisbut);
                var data = {
                    action: 'crawlomatic_activation',
                    code: code,
                    nonce: nonce
                };
                jQuery.post(ajaxurl, data, function(response) {
                    crawlomaticRmLoading(thisbut);
                    if(response.trim() == 'ok')
                    {
                        location.reload();
                    }
